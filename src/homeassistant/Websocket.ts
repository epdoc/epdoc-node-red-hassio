...
export default class Websocket {
  ...
  async connect(): Promise<void> {
    ...
    this.client = await createConnection({
      ...
      this._emitEvent('ha_client:error');
  ...
  _subscribeEntityRegistry() {
    if (atLeastHaVersion(this.client.haVersion, 2023, 3)) {
      subscribeEntityRegistryDisplay(this.client, (entityReg) => {
        ...
        this._emitEvent(HA_EVENT_REGISTRY_UPDATED, {
  ...
  async _haEvents() {
    ...
    await this.client.subscribeEvents<HassEvent>((evt) => this._integrationEvent(evt), HA_EVENT_INTEGRATION);
    ...
    subscribeAreaRegistry(this.client, (areas) => {
      this._emitEvent(HA_EVENT_AREA_REGISTRY_UPDATED, areas);
      this.areas = areas;
    });
    subscribeDeviceRegistry(this.client, (devices) => {
      this._emitEvent(HA_EVENT_DEVICE_REGISTRY_UPDATED, devices);
  ...
  _integrationEvent(evt: Partial<HassEvent>) {
    ...
    switch (evt?.data?.type) {
      ...
      case INTEGRATION_NOT_LOADED:
        this._emitEvent(INTEGRATION_EVENT, evt.data.type);
        return;
    }
  ...
  async subscribeEvents(events: { [nodeId: string]: string }): Promise<void> {
    ...
    if (currentEvents.has('__ALL__')) {
      ...
      this.#unsubCallback.__ALL__ = await this.client.subscribeEvents<HassEvent>((ent) => this.onClientEvents(ent));
    ...
    this.#subscribedEvents = new Set([...[...currentEvents].filter((x) => this.#subscribedEvents.has(x)), ...add]);
    ...
    for (const type of add) {
      this.#unsubCallback[type] = await this.client.subscribeEvents<HassEvent>((ent) => this.onClientEvents(ent), type);
    }
  }
  ...
  onClientStates(entities: HassEntities): void {
    ...
    if (!this.#statesLoaded) {
      ...
      this._emitEvent('ha_client:states_loaded', this.states);
    }
  }
  ...
  onClientServices(services: HassServices): void {
    ...
    this._emitEvent(HA_EVENT_SERVICES_UPDATED, this.services);
    if (!this.#servicesLoaded) {
      this.#servicesLoaded = true;
      this._emitEvent('ha_client:services_loaded');
    }
  }
  ...
  onClientEvents(hassEvent: HassEvent): void {
    ...
    if (eventType === HA_EVENT_STATE_CHANGED) {
      ...
      if (state && entityId) {
        ...
        debug(`Not processing ${HA_EVENT_STATE_CHANGED} event: ${JSON.stringify(event)}`);
        return;
      }
    }
    ...
    if (eventType) {
      this._emitEvent(`${HA_EVENTS}:${eventType}`, event);

      // Most specific emit for event_type and entity_id
      if (entityId) {
        this._emitEvent(`${HA_EVENTS}:${eventType}:${entityId}`, event);
      }
    }

    // Emit on all channel
    this._emitEvent(`${HA_EVENTS}:all`, event);
  }

  async onClientConfigUpdate(config: HassConfig): Promise<void> {
    if (config.components.includes('nodered') && this.integrationVersion === NO_VERSION) {
      try {
  ...
  onClientOpen(): void {
    ...
    if (this.#config.heartbeatInterval) {
      this.#stopHeartbeat = startHeartbeat(this.client, this.#config.heartbeatInterval, this.#config.host);
    }
    this._emitEvent('ha_client:open');
  }
  ...
  onClientError(data: unknown): void {
    ...
    this._emitEvent('ha_client:error', data);
    this.resetClient();
  }
  ...
  resetClient(): void {
    ...
    this._emitEvent('ha_client:close');
  }
  ...
  getAreas(areaId?: unknown): unknown {
    if (areaId) {
      return deepCopy(this.areas.find((area) => area.area_id === areaId));
    }
  ...
  getDevices(deviceId?: unknown): unknown {
    if (deviceId) {
      return deepCopy(this.devices.find((device) => device.id === deviceId));
    }
  ...
  async getDeviceActionCapabilities(action: { [key: string]: any }): Promise<HassDeviceCapabilities> {
  ...
  async getDeviceTriggerCapabilities(trigger: { [key: string]: any }): Promise<HassDeviceCapabilities> {
  ...
  getEntities(entityId?: unknown): unknown {
    if (entityId) {
      return deepCopy(this.entities.find((entity) => entity.entity_id === entityId));
    }
  ...
  getStates(entityId?: unknown): unknown {
    if (typeof entityId === 'string') {
      return this.states[entityId] ? deepCopy(this.states[entityId]) : null;
    }

    return deepCopy(this.states);
  }
  ...
  async getTranslations(category: string, language: string): Promise<HassTranslations> {
