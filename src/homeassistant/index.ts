...
// export function createHomeAssistantClient(node: ServerNode<Credentials>): HomeAssistant {
...
//   });
//   clientEvents.addListener(ClientEvent.Connected, homeAssistant.subscribeEvents.bind(homeAssistant), {
//     once: true
//   });
...
// function createCredentials(credentials: Credentials, config: ServerNodeConfig): Credentials {
//   let host;
//   if (!credentials) {
...
// function createHttpConfig(credentials: Credentials, config: ServerNodeConfig): HttpConfig {
//   return {
...
//   }
// ): WebsocketConfig {
//   const connectionDelay = credentials.host !== SUPERVISOR_URL ? false : config.connectionDelay ?? false;
//   const heartbeatInterval = Number(config.heartbeatInterval) ?? 0;
//   const heartbeat = config.heartbeat && Number.isInteger(heartbeatInterval) ? heartbeatInterval : 0;
...
// export function getHomeAssistant(serverNode: ServerNode<Credentials>): HomeAssistant {
//   return homeAssistantConnections.get(serverNode.id) ?? createHomeAssistantClient(serverNode);
// }
