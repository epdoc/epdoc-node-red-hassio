...
export interface FanImplConstructor {
  node: Node;
  config: FanControlDef;
}

export class FanImpl {
  ...
  constructor(params: FanImplConstructor) {
    this._node = params.node;
    console.log(`fan-control constructor context this = ${JSON.stringify(this._node.context())}`);
    this._ha = new HA(this.global);
    this.setUiConfig(params.config);
  }
