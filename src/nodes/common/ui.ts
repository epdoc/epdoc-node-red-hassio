...
export function createDeviceExtraFields(fields: any[] = [], capabilities: any[] = []) {
  ...
  fields.forEach((field) => {
    const selectedCapabilities = capabilities.find((i) => i.name === field.name);
    switch (field.type) {
      case 'boolean': {
        const element = createDeviceBoolean(field, selectedCapabilities);
      ...
      case 'integer': {
        const element = createDeviceInteger(field, selectedCapabilities);
        elements.push(element);
        break;
      }
      case 'positive_time_period_dict': {
        const element = createDeviceDuration(field, selectedCapabilities);
        elements.push($(element));
        break;
      }
      case 'select': {
...
function createDeviceInteger(field: any, store: any) {
  ...
  const $elements = $(`
        ...
        <div class="range-row">
            <input type="checkbox" />
            <input type="range" id="${id}" max="${field.valueMax}" min="${field.valueMin ?? 0}" value="${value}" />
  ...
  $checkbox
    .on('change', () => {
      $range.prop('disabled', !$checkbox.prop('checked')).trigger('change');
...
function createDeviceSelect(field: Record<string, any>, store: Record<string, any> = {}) {
  const id = `deviceExtra-${field.name}`;
  const options = field.options.reduce((acc: any[], [val, str]: [string, string]) => {
...
function createDeviceString(field: Record<any, string>, store: Record<any, string>) {
  const id = `deviceExtra-${field.name}`;
  const value = store ? store.value : '';
  const html = `
        <label for="${id}">${field.name}</label>
...
function wrapWithRow(ele: unknown, cls: string[] = ['deviceExtra']): unknown {
  if (ele instanceof $) {
    return $(`<div class="form-row ${cls.join(' ')}"></div>`).append(ele as JQuery);
  }
...
export function getCapabilities(capabilities: any[] = []) {
  const properties = capabilities.reduce((acc, item) => {
    ...
    switch (item.type) {
      ...
      case 'positive_time_period_dict':
        if (value && value.length && !isNaN(Number(value))) {
          const cap = {
            ...
            type: item.type,
            value: Number(value),
            unit: item.type === 'positive_time_period_dict' ? $(`#${id}Units`).val() : undefined
          };
          acc.push(cap);
        }
        break;
      case 'integer': {
        const $sel = $(`#${id}`);
        const checked = $sel.siblings('input[type="checkbox"]').prop('checked');
