...
function doPing(node, host, msg, arrayMode) {
  ...
  ex.stderr.on('data', function (data) {
    if (!data.includes('Usage')) {
      // !data: only get the error and not how to use the ping command
      if (data.includes('invalid') && data.includes('6')) {
        //if IPv6 not supported in version of ping try ping6
        tryPing6 = true;
  ...
  ex.on('close', function (code) {
    if (tryPing6 === false) {
      ...
      ex6.stderr.on('data', function (data) {
        if (!data.includes('Usage')) {
          // !data: only get the error and not how to use the ping6 command
