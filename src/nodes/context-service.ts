...
export function isContextLocation(location: string | ContextLocation): location is ContextLocation {
...
export default class NodeRedContextService {
  ...
  get(location: keyof NodeContext | 'msg', property: string, message?: NodeMessage) {
    if (location === 'msg') {
      if (!message) return;
  ...
  set(val: any, location: ContextLocation, property: string, message?: NodeMessage) {
    const { key, store } = RED.util.parseContextStore(property);

    switch (location) {
      case 'flow':
      case 'global':
        this._node.context()[location].set(key, val, store);
        break;
      case 'msg':
        if (message) {
